   ...
   read_api_exit=0
   for  file_name in `aws s3api list-objects-v2 --bucket $line | grep key | awk ' { print $2 } ' | tr -d \",\,`; do aws s3api get-object-acl --bucket $line --key $file_name >/dev/null done  || read_api_exit=$? ; done
